...
export async function fetchCharacters({search = '', page = '1', pageSize = '50'}): Promise<Character[]> {
  const urlParams = new URLSearchParams({page, pageSize});
  if (search) {
    urlParams.append('name', search)
  }
  const newEndpoint = new URL(`${endpoints.characterList()}?${urlParams.toString()}`);
...
export async function fetchFeaturedCharacters(): Promise<Character[]> {
  ...
  const responses: Response[] = await Promise.all(
    featuredIds.map(id => fetch(endpoints.characterDetail(id), {cache: 'force-cache',}))
  )
